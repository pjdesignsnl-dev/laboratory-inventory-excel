# Production build: workbook/LabInventory_v1.0.0-production.xlsm
# - from the frozen macro-free base (D-022 hash-free protection)
# - clears synthetic fixture rows from operational tables (Products, Containers,
#   Transactions, Suppliers, Locations); keeps controlled lists + Settings
# - imports ONLY operational VBA modules (no modTestHooks / modFaultInjection)
# - injects ThisWorkbook + Scan document-module code
# - compiles via save, hides VBE, exports components for verification
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
$SrcXlsx = Join-Path $Root "workbook\LabInventory_v0.1.xlsx"
$DstXlsm = Join-Path $Root "workbook\LabInventory_v1.0.0-production.xlsm"
$ModDir = Join-Path $Root "vba\modules"
$DocDir = Join-Path $Root "vba\docmodules"
$ExportDir = Join-Path $Root "vba\exported-production"
$Log = Join-Path $Root "evidence\vba\build-production-log.txt"
...
try {
    ...
    # ---- clear synthetic fixture rows from operational tables (keep 0 rows) ----
    $clearTables = @("tblProducts", "tblContainers", "tblTransactions", "tblSuppliers", "tblLocations")
    foreach ($tn in $clearTables) {
        foreach ($wsObj in @($wb.Worksheets)) {
            ...
            if ($null -ne $lo) {
                ...
                $wsObj.Protect()
                $lines.Add("cleared " + $tn + " on " + $wsObj.Name)
                break
            }
        }
    }
    ...
    # ---- import operational modules (+ inert modFaultInjection: the
    # FaultAt call-sites in AppendTransaction/ApplyStateChange/AddContainer/
    # ReceiveOne/CommitAction require it to compile. It is INERT in production
    # — faults arm only via modTestHooks, which is absent, so production can
    # never trigger one). ----
